...
class AccountAndGradeFlowIT extends FacadeIT {
  ...
  private void createAdminSafely(String email, String pwd) {
    try {
      accountService.createAdmin(email, pwd);
    } catch (ConflictException ignored) {
    }
  }

  private hei.poja.io.model.Teacher createTeacherSafely(
      String email, String pwd, String first, String last) {
    try {
      return accountService.createTeacher(email, pwd, first, last);
      ...
      var studentResp =
          postAs(
              ...
              new CreateStudentRequest(
                  "student-rest@hei.flow", "pwd", "Grace", "Hopper", Track.EL, ACADEMIC_YEAR));
    ...
    void duplicate_account_returns_conflict() {
      ...
      var first =
          postAs(
              ...
              "/students",
              new CreateStudentRequest(
                  "dup-flow@hei.school", "pwd", "A", "B", Track.EL, ACADEMIC_YEAR));
  ...
  class ExamAndGradingFlow {
    ...
    @Test
    @DisplayName(
        "Teacher creates exam, grades student, forbidden for unassigned teacher, correction"
            + " requires reason")
    void teacher_grades_student_with_full_validation() {
      ...
      var examResponse =
          postAs(
              ...
              "/exams",
              new ExamRequest(
                  courseId(), "Examen final", Instant.now(), BigDecimal.ONE, ACADEMIC_YEAR, 1));
      ...
      var forbidden =
          postAs(
              ...
              "/students/" + studentId() + "/grades",
              new GradeRequest(
                  UUID.fromString(examId), new BigDecimal("15"), "tentative non autorisee"));
      ...
      assertThat(missingReason.getStatusCode().value())
          .as("Correction without reason")
          .isEqualTo(400);
    ...
    void teacher_can_list_exams_for_course() {
      ...
      postAs(
          ...
          "/exams",
          new ExamRequest(
              courseId(), "Examen 2", Instant.now(), new BigDecimal("2"), ACADEMIC_YEAR, 1));

      var exams = getAs(TEACHER_EMAIL, TEACHER_PWD, "/exams?courseId=" + courseId(), List.class);
  ...
  class GroupManagementFlow {
    ...
    void admin_changes_group_with_history_tracking() {
      ...
      var gradesAfterGroupChange =
          getAs(STUDENT_EMAIL, STUDENT_PWD, "/students/" + studentId() + "/grades", List.class);
      assertThat(gradesAfterGroupChange.getBody())
          .as("Grades persist after group change")
          .hasSize(1);
    }
  }
  ...
  private <T> ResponseEntity<T> getAs(
      String email, String password, String path, Class<T> responseType) {
    HttpHeaders headers = new HttpHeaders();
    headers.setBasicAuth(email, password);
