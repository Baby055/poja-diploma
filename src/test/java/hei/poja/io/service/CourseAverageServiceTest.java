...
class CourseAverageServiceTest {
    ...
    @BeforeEach
    void setUp() {
        courseAverageService = new CourseAverageService(courseRepository, examRepository, gradeRepository);
    }
    ...
    void averageForCourse_weights_by_coefficient() {
        ...
        when(gradeRepository.findByStudentIdAndExam_CourseIdAndExam_AcademicYear(studentId, courseId, academicYear))
    ...
    void averagesByCourse_returns_map_of_averages() {
        ...
        when(gradeRepository.findByStudentIdAndExam_CourseIdAndExam_AcademicYear(studentId, course2Id, academicYear))
                .thenReturn(List.of(grade2));

        var result = courseAverageService.averagesByCourse(studentId, List.of(course1, course2), academicYear);
    ...
    void isEligibleForDiploma_false_when_one_course_below_ten() {
        Course course1 = Course.builder().id(UUID.randomUUID()).ref("A").credits(5).track(Track.EL).build();
        Course course2 = Course.builder().id(UUID.randomUUID()).ref("B").credits(5).track(Track.EL).build();
        ...
        when(gradeRepository.findByStudentIdAndExam_CourseIdAndExam_AcademicYear(studentId, course1.id(), academicYear))
                .thenReturn(List.of(grade1));
        when(gradeRepository.findByStudentIdAndExam_CourseIdAndExam_AcademicYear(studentId, course2.id(), academicYear))
                .thenReturn(List.of(grade2));

        assertThat(courseAverageService.isEligibleForDiploma(studentId, List.of(course1, course2), academicYear))
                .isFalse();
    }
    ...
    void isEligibleForDiploma_false_when_no_grade_at_all() {
        ...
        assertThat(courseAverageService.isEligibleForDiploma(studentId, List.of(course), academicYear)).isFalse();
    }
    ...
    void generalAverage_weights_by_credits() {
        Course course1 = Course.builder().id(UUID.randomUUID()).ref("A").credits(5).track(Track.EL).build();
        Course course2 = Course.builder().id(UUID.randomUUID()).ref("B").credits(3).track(Track.EL).build();
        ...
        // (10*5 + 18*3) / 8 = 13.0
        assertThat(courseAverageService.generalAverage(studentId, List.of(course1, course2), academicYear))
