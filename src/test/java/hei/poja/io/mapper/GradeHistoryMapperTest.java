...
class GradeHistoryMapperTest {
    ...
    void toModel_maps_history_entry_with_reason() {
        ...
        JAppUser studentUser = JAppUser.builder().id(studentId).email("etu@hei.school").role(Role.STUDENT).build();
        JStudent student =
                JStudent.builder().id(studentId).user(studentUser).firstName("Grace").lastName("Hopper").track(Track.EL).enrollmentYear(2024).build();
        JCourse course =
                JCourse.builder().id(UUID.randomUUID()).ref("ALG101").title("Algo").credits(5).track(Track.EL).build();
        JExam exam =
                JExam.builder().id(UUID.randomUUID()).course(course).coefficient(BigDecimal.ONE).academicYear(2024).semester(1).build();
        JGrade grade =
                JGrade.builder().id(UUID.randomUUID()).student(student).exam(exam).value(new BigDecimal("14")).lastModifiedAt(Instant.now()).build();
        JAppUser modifier = JAppUser.builder().id(UUID.randomUUID()).email("prof@hei.school").role(Role.TEACHER).build();
