...
class TeacherMapperTest {
    ...
    void toModel_maps_teacher_and_nested_user() {
        ...
        JTeacher entity = JTeacher.builder().id(id).user(user).firstName("Ada").lastName("Lovelace").build();
    ...
    void toEntity_maps_teacher_and_nested_user() {
        ...
        hei.poja.io.model.AppUser user =
                hei.poja.io.model.AppUser.builder().id(id).email("prof@hei.school").role(Role.TEACHER).build();
        Teacher model = Teacher.builder().id(id).user(user).firstName("Alan").lastName("Turing").build();
