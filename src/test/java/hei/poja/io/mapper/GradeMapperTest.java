...
class GradeMapperTest {
    ...
    void toModel_maps_grade_with_nested_student_and_exam() {
        ...
        JExam exam =
                JExam.builder().id(UUID.randomUUID()).course(course).title("Final").coefficient(BigDecimal.ONE).academicYear(2024).semester(1).build();
        JGrade entity =
                JGrade.builder().id(UUID.randomUUID()).student(student).exam(exam).value(new BigDecimal("14.5")).lastModifiedAt(Instant.now()).build();
