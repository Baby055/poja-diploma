...
class CourseMapperTest {
    ...
    void toModel_handles_null_track_as_tronc_commun() {
        JCourse entity =
                JCourse.builder().id(UUID.randomUUID()).ref("COM101").title("Commun").credits(2).track(null).build();
    ...
    void toEntity_maps_all_fields() {
        UUID id = UUID.randomUUID();
        Course model = Course.builder().id(id).ref("RES101").title("Reseaux").credits(4).track(Track.TN).build();
